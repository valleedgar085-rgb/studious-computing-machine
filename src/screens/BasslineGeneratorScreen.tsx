...
export const BasslineGeneratorScreen: React.FC = () => {
  ...
  const generatePattern = () => {
    const newPattern = patternFactory.generateBasslinePattern(density, octave);
    setPattern(newPattern);
  };

  const handleDensityChange = (newDensity: number) => {
    setDensity(newDensity);
    const newPattern = patternFactory.generateBasslinePattern(newDensity, octave);
    setPattern(newPattern);
  };

  const handleOctaveChange = (newOctave: number) => {
    setOctave(newOctave);
    const newPattern = patternFactory.generateBasslinePattern(density, newOctave);
    setPattern(newPattern);
  };
